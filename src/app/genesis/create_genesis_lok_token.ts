...
import { SIGMALOK_CONTRACT_TREE } from '../contract';
import { getInputBoxes } from '../../utils/input-selector';

async function createGenesisTokenLok() {
  ...
  const deadlineHeight = BigInt(blockHeight + 10);
  const contractName = "Kushti's SigmaLok";
  ...
  const tokenToLockAmount = BigInt(2);
